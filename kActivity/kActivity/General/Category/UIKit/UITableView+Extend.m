...
- (void)setNeedHeaderRefresh {
    ...
    headerView.y = 0-headerView.height;
...
- (void)setNeedTailerRefresh {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if ([delegate refreshTailerView].superview) {
            ...
            view.y = self.contentSize.height;
        }
    });
}
- (void)removeRefresh {
...
- (void)refreshTableViewDidScroll {
    ...
    } else if (ceil(self.contentOffset.y) >= floor(self.contentSize.height - self.height + tailerToggleHeight)) {
...
- (void)setRefreshState:(TableRefreshState)state isHeader:(BOOL)isHeader {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        if (isHeader) {
            ...
            weakSelf.contentOffset = CGPointMake(0, weakSelf.contentSize.height - weakSelf.height + tailerToggleHeight);
        }
    }];
}
@end
